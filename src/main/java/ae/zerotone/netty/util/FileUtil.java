...
public class FileUtil {
  ...
  public static FileBurstData readFile(String fileUrl, Integer readPosition) throws IOException {
    ...
    byte[] bytes = new byte[Constants.READ_BLOCK_SIZE];
    int readSize = randomAccessFile.read(bytes);
    if (readSize <= 0) {
      randomAccessFile.close();
      return new FileBurstData(
          Constants.FileStatus.COMPLETE); // Constants.FileStatus ｛0开始、1中间、2结尾、3完成｝
    }
    ...
    // 不足1024需要拷贝去掉空字节
    if (readSize < Constants.READ_BLOCK_SIZE) {
  ...
  public static FileBurstInstruct writeFile(String baseUrl, FileBurstData fileBurstData)
    ...
    randomAccessFile.write(
        fileBurstData.getBytes()); // 调用了seek（start）方法，是指把文件的记录指针定位到start字节的位置。也就是说程序将从start字节开始写数据
    randomAccessFile.close();
    ...
    fileBurstInstruct.setStatus(
        Constants.FileStatus.CENTER); // Constants.FileStatus ｛0开始、1中间、2结尾、3完成｝
