...
import ae.zerotone.netty.util.CacheUtil;
  ...
  public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
    ...
    switch (fileTransferProtocol.getTransferType()) {
      case 0: // 收到客户端文件传输请求
        ...
        FileBurstInstruct fileBurstInstructOld =
            CacheUtil.burstDataMap.get(fileDescInfo.getFileName());
        if (null != fileBurstInstructOld) {
          ...
          System.out.println(
              new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
                  + " File manage 服务端，接收客户端传输文件请求[断点续传]。"
        ...
        FileTransferProtocol sendFileTransferProtocol =
            MsgUtil.buildTransferInstruct(Constants.FileStatus.BEGIN, fileDescInfo.getFileUrl(), 0);
        ctx.writeAndFlush(sendFileTransferProtocol);
        System.out.println(
            new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())
                + "File manage 服务端，接收客户端传输文件请求。"
      ...
      case 2: // 收到客户端文件传输数据
        ...
        FileBurstInstruct fileBurstInstruct =
            FileUtil.writeFile(
                "/Users/andying/github/maloong-zhang/netty/file-upload/data/", fileBurstData);
